      ...
      <section className="container-page pb-12 sm:pb-16">
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-3">
          {principles.map((principle) => (
            <article
              key={principle.title}
              className="rounded-lg border border-karimoff-line bg-white p-5 shadow-card transition hover:-translate-y-0.5 hover:border-karimoff-orange/40 sm:p-6"
            >
              <div className="mb-5 h-1.5 w-12 rounded-full bg-karimoff-orange" />
              <h3 className="text-xl font-black">{principle.title}</h3>
              <p className="mt-3 text-[15px] leading-7 text-karimoff-muted">{principle.text}</p>
            </article>
          ))}
        </div>
      </section>
