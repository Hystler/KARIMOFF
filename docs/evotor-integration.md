...
> Аналитический слой: импортированные чеки читаются через provider-neutral views `analytics_sales`, `analytics_sale_items` и `analytics_sale_payments`. Интеграция остаётся read-only: аналитика не изменяет товары, цены, остатки или склад. Подробные определения находятся в `docs/karimoff-unified-analytics.md`.
...
- `401 Unauthorized` означает ошибку аутентификации application token. `403 Forbidden` означает, что приложению не выдано нужное разрешение или приложение недоступно для этой установки; такой ответ не помечает сохранённый token как отозванный.
...
## Разрешения тестовой версии

Для минимальной проверки подключения включите в разделе «Запросы к API Эвотор» только:

- **«Получать список магазинов из облака Эвотор»** (`partner.get.stores`);
- **«Получать список устройств из облака Эвотор»** (`partner.get.devices`).

Для последующей полной синхронизации чтения также понадобятся:

- **«Получать список сотрудников из облака Эвотор»** (`partner.get.employees`);
- **«Получать номенклатуру из облака Эвотор»** (`partner.get.v1.inventories.stores.products`);
- **«Получать документы из облака Эвотор»** (`partner.get.api.v1.inventories.stores.documents`).

Разрешения **«Управлять номенклатурой в облаке Эвотор»** и **«Изменить остатки номенклатуры в Облаке Эвотор»** на read-only этапе должны оставаться выключенными. Проверка соединения выполняет только `GET /stores` и `GET /devices`.
...
7. В «Запросах к API Эвотор» включите точные read-only разрешения из раздела выше. Не включайте изменение товаров, цен и остатков.
